...
export default function LandingPage() {
  return (
    <main className="flex min-h-screen flex-col bg-white">
      ...
      {/* Nav */}
      <nav className="flex items-center justify-between px-6 py-4 border-b border-zinc-200 sticky top-0 bg-white z-50">
        ...
        <div className="hidden md:flex items-center gap-6 text-sm text-zinc-500">
          <Link href="/learn" className="hover:text-zinc-900 transition-colors">Browse Courses</Link>
          <Link href="#employers" className="hover:text-zinc-900 transition-colors">For Employers</Link>
          <a href="https://github.com/harleysmodernlife/groundwork" target="_blank" rel="noopener noreferrer" className="hover:text-zinc-900 transition-colors">GitHub</a>
        </div>
      ...
      <section className="flex flex-col items-center justify-center text-center px-6 py-28 bg-white">
        <span className="inline-block text-xs font-semibold uppercase tracking-widest text-zinc-400 border border-zinc-200 rounded-full px-4 py-1.5 mb-6">
          100% free · No catch · No paywall
        </span>
        <h1 className="text-5xl md:text-6xl font-bold tracking-tight text-zinc-900 max-w-3xl leading-tight mb-6">
      ...
      <section className="px-6 py-24 max-w-6xl mx-auto w-full">
        <div className="text-center mb-12">
          ...
          <p className="text-zinc-500">Every subject. Every course. All free.</p>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
          {SUBJECTS.map((s) => (
            <Link
              key={s.slug}
              href={`/learn/${s.slug}`}
              className="flex items-center gap-3 px-4 py-3 rounded-xl border border-zinc-200 hover:border-zinc-400 hover:shadow-sm transition-all group"
            >
              <span className="text-2xl">{s.icon}</span>
              <div className="min-w-0">
                <p className="text-sm font-medium text-zinc-900 leading-tight group-hover:text-zinc-700">{s.name}</p>
                <p className="text-xs text-zinc-400 mt-0.5">{s.courses} courses</p>
              </div>
            </Link>
          ))}
        </div>
      ...
      <section className="bg-zinc-50 border-t border-zinc-200 px-6 py-24">
        <div className="max-w-4xl mx-auto">
          <h2 className="text-3xl font-bold text-center mb-16">How Groundwork works</h2>
      ...
      <section className="px-6 py-24 max-w-4xl mx-auto w-full">
        <div className="grid md:grid-cols-2 gap-12 items-center">
          <div>
            <p className="text-xs font-semibold uppercase tracking-widest text-zinc-400 mb-4">Certificates</p>
            <h2 className="text-3xl font-bold mb-4">Your certificate means something</h2>
      ...
      {/* For Employers */}
      <section id="employers" className="bg-zinc-50 border-t border-zinc-200 px-6 py-24">
        <div className="max-w-4xl mx-auto grid md:grid-cols-2 gap-12 items-center">
          <div>
            <p className="text-xs font-semibold uppercase tracking-widest text-zinc-400 mb-4">For Employers</p>
            <h2 className="text-3xl font-bold mb-4">Find candidates who actually know their stuff</h2>
            <p className="text-zinc-500 leading-relaxed mb-6">
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={card.course} className="bg-white border border-zinc-200 rounded-xl px-5 py-4">
                <p className="font-medium text-sm">{card.course}</p>
                <p className="text-xs text-zinc-400 mt-0.5">{card.subject} · {card.location}</p>
              </div>
            ))}
            <p className="text-xs text-zinc-400 text-center pt-2">Example learner certificates</p>
          </div>
        </div>
      </section>
      ...
      {/* Footer */}
      <footer className="border-t border-zinc-200 px-6 py-12 bg-white">
        <div className="max-w-4xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-8 text-sm mb-10">
          <div>
            <p className="font-semibold text-zinc-900 mb-3">Learners</p>
            <div className="flex flex-col gap-2 text-zinc-500">
              <Link href="/learn" className="hover:text-zinc-900">Browse courses</Link>
              <Link href="/signup" className="hover:text-zinc-900">Create account</Link>
              <Link href="/login" className="hover:text-zinc-900">Log in</Link>
              <Link href="/forgot-password" className="hover:text-zinc-900">Reset password</Link>
            </div>
          </div>
          <div>
            <p className="font-semibold text-zinc-900 mb-3">Employers</p>
            <div className="flex flex-col gap-2 text-zinc-500">
              <Link href="/employer/signup" className="hover:text-zinc-900">Create account</Link>
              <Link href="/employer/login" className="hover:text-zinc-900">Log in</Link>
              <Link href="/employer/search" className="hover:text-zinc-900">Search candidates</Link>
              <Link href="/employer/jobs" className="hover:text-zinc-900">Post a job</Link>
            </div>
          </div>
          <div>
            <p className="font-semibold text-zinc-900 mb-3">Platform</p>
            <div className="flex flex-col gap-2 text-zinc-500">
              <a href="https://github.com/harleysmodernlife/groundwork" target="_blank" rel="noopener noreferrer" className="hover:text-zinc-900">GitHub</a>
              <Link href="/certificates/verify/example" className="hover:text-zinc-900">Verify a certificate</Link>
            </div>
          </div>
          <div>
            <p className="font-bold text-zinc-900 mb-2">Groundwork</p>
